...
function runGroupedTask {
    param (
        ...
        #
        [string]$groupingProperty="DefaultProperty",
        [int]$maxJobs = 11,
        [int]$jobDelay = 30,
...
if (($groupingProperty -eq "") -or ($groupingProperty -eq "DefaultProperty")) {
    $groupingProperty = ((Import-Csv -Path $filePath)[0].PSObject.Properties.Name)[0]
}
...
runGroupedTask -filePath "..\func4ParallelJob\vms.csv" -taskScript $tasker -groupingProperty "subscription" -maxJobs 11