...
$task = {
    try {
        ...
        Write-Output $expression.ToString()
        Write-Output 'Running the expression'
        Invoke-Expression ($expression.ToString() -replace '<MSG>', $stringToPass)
        # or
        # $theScriptBlock [scriptblock]::Create(($expression.ToString() -replace '<MSG>', $stringToPass))
