...
<#
.SYNOPSIS
    Executes the patch report processing for a list of VM objects.
.DESCRIPTION
    This function is refactored from the original `updateReport.ps1` script.
    It accepts a collection of PowerShell objects (e.g., the result of `Import-Csv <file>`)
    instead of a CSV file path.
.PARAMETER VmList
    List of VM objects with properties: Name, ResourceGroup, Subscription, and any custom options.
.PARAMETER MaxJob
    Number of parallel jobs to run. Default is 11.
.PARAMETER ReportPath
    Destination directory for final reports. If not provided, a temporary folder under the working directory is used.
#>
    [CmdletBinding()]
    param (
...
# Export-ModuleMember -Function Invoke-PatchReport
...
$resp = Read-Host
if ($resp -in @('y',"Y")) {
    Invoke-PatchReport -VmList $retryInputs -MaxJob 20 -runName "Run-Retry-$(Get-Date -Format 'dd-MM-yyyyThh-mm')"
