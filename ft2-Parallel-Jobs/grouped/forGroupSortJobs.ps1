...
if ($groupingProperty.Trim() -ne $sortingProperty.Trim()) {
    $VmGroups = $allVms | Group-Object -Property $groupingProperty
}
else {
    if ($ascendingOrder) {
        $VmGroups = $allVms | Group-Object -Property $groupingProperty | Sort-Object -Property $sortingProperty
    } else {
        $VmGroups = $allVms | Group-Object -Property $groupingProperty | Sort-Object -Property $sortingProperty -Descending
    }
...
foreach ($VmGroup in $VmGroups) {
    ...
    $groupedVMs = $VmGroup.Group
    if ($groupingProperty.Trim() -ne $sortingProperty.Trim()) {
        if ($ascendingOrder) {
            $groupedVMs = $VmGroup.Group | Sort-Object -Property $sortingProperty
        }
        else {
            $groupedVMs = $VmGroup.Group | Sort-Object -Property $sortingProperty -Descending
        }
    }
    
    # $groupedVMs = $VmGroup | Sort-Object -Property $sortingProperty
    runParaJobs -maxJob $jobsToRun -taskScript $task -vms $groupedVMs -groupingName $VmGroup.Name -runName $RunName
    Start-Sleep -Seconds 11
}
