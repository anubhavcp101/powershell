...
$task = {
    ...
    #
    $dCtx = Set-AzContext -Subscription $disk.diskSubscription.trim() -ErrorAction Stop
    $filePath = ".\snapshots.csv"
    $azDisk = Get-AzDisk -DiskName $disk.diskName.trim() -ResourceGroupName $disk.diskRG.trim() -DefaultProfile $dCtx
    ...
    $snpCtx = Set-AzContext -Subscription $disk.snapshotSubscription.trim()
    if ($disk.snapshotName) {
        try {
            New-AzSnapshot -SnapshotName $disk.snapshotName.trim() -ResourceGroupName $disk.snapshotRG.trim() -Snapshot $snapshotConfig -DefaultProfile $snpCtx
        ...
        try {
            New-AzSnapshot -SnapshotName ("snpsht-$($azdisk.Name)") -ResourceGroupName $disk.snapshotRG.trim() -Snapshot $snapshotConfig -DefaultProfile $snpCtx
