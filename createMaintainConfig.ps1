...
function createMaintainConfig {
    ...
    if ($rebootOpt -in $rebootOptions) {
        ...
        New-AzMaintenanceConfiguration -ResourceGroupName $RGName -Name $configName -MaintenanceScope $scope -Location $location -StartDateTime $startDateTime -ExpirationDateTime $ExpirationDateTime -Timezone $timeZone -Duration $duration -RecurEvery $recurEvery -WindowParameterClassificationToInclude $WindowsParameterClassificationToInclude -LinuxParameterClassificationToInclude $LinuxParameterClassificationToInclude -InstallPatchRebootSetting $RebootOption -ExtensionProperty @{"InGuestPatchMode" = "User" }
    }
    else {
