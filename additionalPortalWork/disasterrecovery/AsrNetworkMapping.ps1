...
foreach ($vm in $vms) {
    ...
    -primaryVMId $vm.primaryVMId `
    -vaultSubscription $vm.vaultSubscription `
    -vaultName $vm.vaultName `
    -primaryRegion $vm.primaryRegion `
    -secondaryRegion $vm.secondaryRegion `
    -recoveryVnetId $vm.recoveryVnetId
