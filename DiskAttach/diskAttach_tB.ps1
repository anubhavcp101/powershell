function diskCreate {
    ...
    try {
        # Create a new DataDisk object to represent the new disk
        $newDataDisk = New-Object Microsoft.Azure.Management.Compute.Models.DataDisk
        $newDataDisk.Lun = $newLun
        $newDataDisk.Name = $newDiskName
        $newDataDisk.DiskSizeGB = $newSizeGB
        $newDataDisk.ManagedDisk = @{ Id = $newDisk.Id }
        $newDataDisk.CreateOption = "Attach"
        $newDataDisk.Caching = $oldHostCaching

        # Add the new disk to the VM
        $vm.StorageProfile.DataDisks += $newDataDisk
