#
<#
This needs a csv file named vms.csv having Name and mcId as its header where Name is name of the VM and mcId is resource Id of the
maintenance configuration to which the corresponding VM should be part of.
#>
$maxJobCount = 11
...
$task = {
    ...
    if ($resIdCount -gt 0 -and $resIdCount -lt 2) {
        ...
        $configAssignmentName = (($vm.mcId -split("/"))[-1] + "assignment")
        New-AzConfigurationAssignment -ResourceGroupName $AzVm.ResourceGroupName -Location $AzVm.Location -ResourceName $AzVm.Name -ResourceType "VirtualMachines" -ProviderName "Microsoft.Compute" -ConfigurationAssignmentName $configAssignmentName -MaintenanceConfigurationId $vm.mcId
    } else {
